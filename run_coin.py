...
st.set_page_config(page_title="코인 마스터 V71.2", page_icon="🪙", layout="wide")
...
def get_data_safe(ticker, days=200):
    ...
    today_str = datetime.datetime.now().strftime('%Y%m%d_%H')
...
def get_coin_listing():
    try:
        ...
        return tickers, f"⚡ 업비트 KRW ({len(tickers)}개)"
    except:
        return ['KRW-BTC', 'KRW-ETH', 'KRW-XRP', 'KRW-SOL'], "⚠️ 백업 리스트"
...
def get_darwin_strategy(df, buy_price=0):
    ...
    ai_prob = 50; logic_mode = "⚖️ 균형 모드"; top_feature = "없음"
    if len(data_ml) > 60:
        try:
    ...
    if is_bull_setup:
        ...
        if cp >= curr['MVWAP']: score += 15; hit_reasons.append("고래매집")
        ...
        if cp <= curr['OB'] * 1.05: score += 20; hit_reasons.append("오더블럭지지")
        if curr['MFI'] < 20: score += 10; hit_reasons.append("MFI바닥")
        if curr['Squeeze']: score += 15; hit_reasons.append("변동성응축")

    if curr['ER'] > 0.6: score += 10; hit_reasons.append("추세효율좋음")
    ...
    if curr['RSI'] > 80: sell_score += 20; sell_reasons.append("⚠️ RSI초과열")
    if curr['MFI'] > 85: sell_score += 20; sell_reasons.append("⚠️ MFI초과열")
    if cp > curr['BB_Up'] * 1.05: sell_score += 20; sell_reasons.append("⚠️ 볼린저상단돌파")
    
    if is_bull_setup:
        if cp < curr['MA5']: sell_score += 10; sell_reasons.append("📉 5일선이탈")
        if cp < curr['MA20']: sell_score += 50; sell_reasons.append("⛔ 20일선붕괴")
    else:
        if cp < prev['Low']: sell_score += 20; sell_reasons.append("📉 전저점붕괴")
    ...
    targets = []
    targets.append((adj(curr['BB_Up']), "볼린저 상단"))
    if is_bull_setup:
        targets.append((adj(cp * 1.05), "추세+5%"))
        targets.append((adj(cp * 1.10), "추세+10%"))
    else:
        targets.append((adj(curr['MA60']), "60일선 저항"))
        targets.append((adj(cp + atr*3), "기술적 반등"))
    
    sell_pts = sorted(list(set(targets)), key=lambda x: x[0])
    if len(sell_pts) < 3: sell_pts += [(adj(sell_pts[-1][0]*1.03), "고점")] * (3-len(sell_pts))
    sell_pts = sell_pts[:3]

    if is_bull_setup:
        final_buys = [(adj(curr['MA5']), "5일선"), (adj(curr['MA10']), "10일선"), (adj(curr['MA20']), "20일선")]
        final_buys = [x for x in final_buys if x[0] < cp * 1.005]
        if not final_buys: final_buys = [(adj(cp), "현재가"), (adj(curr['MA5']), "5일선"), (adj(curr['MA10']), "10일선")]
        elif len(final_buys) < 3: 
             while len(final_buys) < 3: final_buys.append((adj(final_buys[-1][0]*0.98), "불타기"))
    else:
        candidates = [(adj(curr['MVWAP']), "고래평단"), (adj(curr['OB']), "주요지지"), (adj(curr['Fibo_618']), "피보나치"), (adj(curr['BB_Lo']), "볼린저하단")]
        valid = [x for x in candidates if x[0] <= cp]
        final_buys = valid[:3] if len(valid)>=3 else (valid + [(adj(cp*0.95), "저점")]*3)[:3]

    final_buys.sort(key=lambda x: x[0], reverse=True)
    est_avg = sum([p[0] for p in final_buys]) / 3
    est_avg = adj(est_avg)
    
    status = {"type": "💤 관망 중", "color": "#78909c", "msg": "매수 대기", "alert": False}
    if sell_score >= 50:
        status = {"type": "🔴 매도 경고", "color": "#d32f2f", "msg": f"{sell_reasons[0] if sell_reasons else '위험'} 발생", "alert": True}
    elif buy_price > 0:
        pct = (cp - buy_price) / buy_price * 100
        if sell_score >= 30: status = {"type": "⚠️ 주의 필요", "color": "#f57f17", "msg": "분할매도 권장", "alert": True}
        elif pct > 0: status = {"type": "💰 익절 구간", "color": "#2e7d32", "msg": f"수익 +{pct:.1f}%", "alert": False}
        elif pct < -3: status = {"type": "❄️ 손실 구간", "color": "#1976d2", "msg": "존버 (홀딩)", "alert": False}
...
with st.sidebar:
    ...
    st.markdown('<div class="status-open">🟣 업비트 24/7 라이브</div>', unsafe_allow_html=True)
    ...
    with st.expander("⚙️ 설정 및 알림", expanded=True):
        tg_token = st.text_input("텔레그램 토큰", type="password")
        tg_id = st.text_input("텔레그램 챗ID")
        st.markdown("---")
        auto_report = st.checkbox("✅ 아침 정기 리포트", value=True)
        report_time = st.time_input("발송 시간 설정", datetime.time(9, 0))
        scanner_alert = st.checkbox("📢 스캔 포착 시 알림", value=True)
        st.markdown("---")
        auto_refresh = st.checkbox("🔄 자동 새로고침 (PC용)", value=False)
        refresh_min = st.slider("새로고침 주기(분)", 1, 60, 5)
    
    if auto_report and now.hour == report_time.hour and now.minute == report_time.minute:
        pf_rep = get_portfolio_gsheets()
        if not pf_rep.empty:
            msg = f"🛡️ <b>[{report_time.strftime('%H:%M')} 코인 정기 브리핑]</b>\n"
...
tabs = st.tabs(["📊 대시보드", "🔍 종목 발굴", "🧬 과거 검증", "💼 AI 리포트", "➕ 포트폴리오"])
...
with tabs[0]: # 대시보드
    ...
    if not pf.empty:
        ...
        for _, row in pf.iterrows():
            ...
            if d is not None:
                last_source = d.attrs.get('source', '알 수 없음')
                ...
                dash_list.append({"코인": row['Name'], "수익금": (cp-row['Buy_Price'])*row['Qty'], "상태": res['status']['type']})
        
        source_container.markdown(f'<div class="source-box">📡 {last_source}</div>', unsafe_allow_html=True)
        
        c1, c2, c3 = st.columns(3)
        c1.metric("총 매수 금액", f"{int(t_buy):,}원")
        c2.metric("총 평가 금액", f"{int(t_eval):,}원", f"{(t_eval-t_buy)/t_buy*100:+.2f}%" if t_buy>0 else "0%")
        c3.metric("평가 손익", f"{int(t_eval-t_buy):,}원")
        if dash_list: st.plotly_chart(px.bar(pd.DataFrame(dash_list), x='코인', y='수익금', color='상태', template="plotly_white", title="보유 코인 수익 현황"), use_container_width=True)
    else:
        st.info("👈 왼쪽 사이드바 또는 '포트폴리오' 탭에서 보유 코인을 등록해주세요.")
    
    if auto_refresh:
        time.sleep(refresh_min * 60); st.rerun()

with tabs[1]: # 스캐너
    st.subheader("🚀 AI 추천 코인 스캐너")
    if st.button("🛡️ 전 종목 스캔 시작") or auto_refresh:
        if auto_refresh: st.info(f"🔄 자동 스캔 중... (주기: {refresh_min}분)")
        
        targets_code = tickers_list[:40] # 속도를 위해 40개만 샘플링
            ...
            st.markdown(f"""
                <div class="scanner-card">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <div><h3 style="margin:0;">{d['name']}</h3><span class="current-price">{d['cp']:,}원</span><span class="pro-tag" style="margin-left:5px;">고래평단: {s['mvwap']:,}</span></div>
                        <div style="text-align:right;"><span class="ai-badge">AI확률: {s['ai']}%</span><span style="font-size:1.1em; font-weight:bold; color:#5e35b1; margin-left:5px;">점수: {s['score']}</span><br><span class="mode-badge" style="font-size:0.8em; margin-top:5px; display:inline-block;">{s['logic']}</span></div>
                    </div>
                    ...
                    <div style="margin: 10px 0; display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
                        <div class="buy-box"><b>🔵 스마트 진입 (매수)</b><br>1차: <b>{s['buy'][0][0]:,}원</b> <span class="logic-tag">{s['buy'][0][1]}</span><br>2차: <b>{s['buy'][1][0]:,}원</b> <span class="logic-tag">{s['buy'][1][1]}</span><br>3차: <b>{s['buy'][2][0]:,}원</b> <span class="logic-tag">{s['buy'][2][1]}</span><div class="avg-text">예상 평단가: {s['avg']:,}원</div></div>
                        <div class="sell-box"><b>🔴 스마트 청산 (매도)</b><br>1차: {s['sell'][0][0]:,}원 <span class="logic-tag">{s['sell'][0][1]}</span><br>2차: {s['sell'][1][0]:,}원 <span class="logic-tag">{s['sell'][1][1]}</span><br>3차: {s['sell'][2][0]:,}원 <span class="logic-tag">{s['sell'][2][1]}</span></div>
                    </div>
...
with tabs[2]: # 5년 검증
    ...
    if st.button("🚀 검증 시작"):
        ...
        if not targets: st.error("검증할 종목이 없습니다.")
        else:
            results = []
            prog = st.progress(0)
            for idx, code in enumerate(targets):
                status_text.write(f"⏳ **[{idx+1}/{len(targets)}] {code}** 과거 차트 분석 중...")
                time.sleep(0.1) 
                full_df_raw = get_data_safe(code, days=1000)
            ...
            if results:
                ...
                c1, c2 = st.columns(2)
                c1.metric("총 시뮬레이션 횟수", f"{len(df_res)}회"); c2.metric("누적 승률", f"{df_res['Win_Rate'].iloc[-1]:.1f}%")
                fig = px.line(df_res, x='Date', y='Win_Rate', title="시간 경과에 따른 AI 승률 변화", markers=False)
                fig.add_hline(y=50, line_dash="dot", line_color="gray"); st.plotly_chart(fig, use_container_width=True)
            else: st.error("분석 가능한 데이터가 부족합니다.")

with tabs[3]: # AI 리포트
    st.subheader("💼 종목 상세 AI 리포트")
    if not pf.empty:
        sel = st.selectbox("분석할 코인 선택", pf['Name'].unique())
        ...
        if raw_df is not None:
            ...
            if st.button("📡 내 텔레그램으로 전략 전송"):
                msg = f"🛡️ <b>[{sel}] AI 전략</b>\n💰 현재가: {cp:,}원\n\n🔵 1차 매수: {res['buy'][0][0]:,}원\n🔴 1차 매도: {res['sell'][0][0]:,}원\n💡 예상 평단: {res['avg']:,}원"
            ...
            buy_html = f"""<div class="buy-box"><b>🔵 스마트 진입 (매수)</b><br>1차: <b>{res['buy'][0][0]:,}원</b> ({res['buy'][0][1]})<br>2차: <b>{res['buy'][1][0]:,}원</b> ({res['buy'][1][1]})<br>3차: <b>{res['buy'][2][0]:,}원</b> ({res['buy'][2][1]})<div class="avg-text">예상 평단가: {res['avg']:,}원</div></div>"""
            sell_html = f"""<div class="sell-box"><b>🔴 스마트 청산 (매도)</b><br>1차: <b>{res['sell'][0][0]:,}원</b> ({res['sell'][0][1]})<br>2차: <b>{res['sell'][1][0]:,}원</b> ({res['sell'][1][1]})<br>3차: <b>{res['sell'][2][0]:,}원</b> ({res['sell'][2][1]})</div>"""
            
            st.markdown(f"""<div class="metric-card" style="border-left:10px solid {res['status']['color']};"><div style="display:flex; justify-content:space-between;"><div><h2>{sel} <span class="mode-badge">{res['logic']}</span></h2><p style="font-size:1.1em; color:{res['status']['color']}; font-weight:bold;">{res['status']['msg']} (AI 승률 예측: {res['ai']}%)</p></div><div style="text-align:right;"><h2 style="color:#333;">{cp:,}원</h2><span class="pro-tag">고래 평단: {int(res['mvwap']):,}</span></div></div><div style="margin:5px 0;">{reasons_html}</div><div>{sell_reasons_html}</div><div style="display:grid; grid-template-columns: 1fr 1fr; gap:15px; margin-top:20px;">{buy_html} {sell_html}</div></div>""", unsafe_allow_html=True)
            ...
            fig.add_hline(y=res['mvwap'], line_color="orange", line_width=2, annotation_text="고래평단(MVWAP)")
            fig.update_layout(height=450, template="plotly_white", xaxis_rangeslider_visible=False, title=f"{sel} 최근 100일 차트")
            st.plotly_chart(fig, use_container_width=True)
    else:
        st.info("먼저 '포트폴리오' 탭에서 코인을 등록해주세요.")
...
with tabs[4]: # 관리
    ...
    with st.form("add"):
        c1, c2, c3 = st.columns(3)
        n = c1.text_input("코인 티커 (예: BTC, DOGE)")
        p = c2.number_input("평균 단가 (원)", min_value=0.0)
        q = c3.number_input("보유 수량", min_value=0.0)
        if st.form_submit_button("내 코인 등록하기"):
            code = n.upper()
            ...
            if code in tickers_list:
                ...
                st.error("업비트에 상장되지 않은 코인입니다.")
    
    if not df_p.empty:
        st.dataframe(df_p, use_container_width=True)
    else:
        st.write("등록된 코인이 없습니다.")
