...
st.set_page_config(page_title="주식 비서 V64.6 Triple Compound", page_icon="⚡", layout="wide")
...
def get_strategy(df, buy_price=0):
    ...
    pyramiding = {"type": "💤 관망", "msg": f"{buy_names[0]} 근접 대기", "color": "#6c757d", "alert": False}
    if buy_price > 0:
        y = (cp - buy_price) / (buy_price + 1e-9) * 100
        if cp >= sell[0]: pyramiding = {"type": "💰 익절", "msg": f"수익률 {y:.1f}% 달성! 분할 매도 구간", "color": "#28a745", "alert": True}
    ...
    return {"buy": buy, "buy_names": buy_names, "sell": sell, "sell_names": sell_names, "stop": stop_loss, "regime": curr['Regime'], 
            "rsi": curr['RSI'], "poc": poc, "ob": ob, "fibo": f618, "bb_l": bbl, "pyramiding": pyramiding}
...
with st.sidebar:
    ...
    alert_scanner = st.checkbox("상세 알림 수신", value=True)
    ...
    if not portfolio.empty:
        ...
        for _, row in portfolio.iterrows():
            ...
            if idx_df is not None:
                ...
                if alert_scanner and st_res['pyramiding']['alert']:
                    has_alert = True
...
# --- [🔍 탭 2: 전략 스캐너 (Speed Engine + Triple UI)] ---
with tabs[2]:
...
with tabs[3]:
    ...
    if st.button("📊 트리플 백테스트 시작"):
        ...
        if not match.empty:
            with st.spinner('실전형 분할 매매 시나리오 가동 중...'):
                ...
                if df_bt is not None:
                    ...
                    st.plotly_chart(px.line(edf, x='date', y='total', title="트리플 분할 매매 자산 성장 곡선"), use_container_width=True)
