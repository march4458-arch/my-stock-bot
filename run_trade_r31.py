...
st.set_page_config(page_title="주식 비서 V64.6 Triple Master", page_icon="⚡", layout="wide")

# UI 전문 디자인 CSS (V64.5 스타일 복구)
st.markdown("""
    <style>
...
with tabs[1]:
    ...
    if not portfolio.empty:
        ...
        if df_ai is not None:
            ...
            with col_b: st.markdown(f'<div class="buy-box"><b>🔵 유기적 3분할 매수</b><br>1차({st_res["buy_names"][0]}): {st_res["buy"][0]:,}원<br>2차({st_res["buy_names"][1]}): {st_res["buy"][1]:,}원<br>3차({st_res["buy_names"][2]}): {st_res["buy"][2]:,}원</div>', unsafe_allow_html=True)
            with col_s: st.markdown(f'<div class="sell-box"><b>🔴 유기적 3분할 매도</b><br>1차: {st_res["sell"][0]:,}원<br>2차: {st_res["sell"][1]:,}원<br>3차: {st_res["sell"][2]:,}원</div>', unsafe_allow_html=True)
        ...
        for idx, d in enumerate(found):
            ...
            st.markdown(f"""<div class="scanner-card" style="border-left: 8px solid {acc_c};">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-top:10px;">
                    <div class="buy-box"><b>🔵 유기적 3분할 매수</b><br>1차({d['strat']['buy_names'][0]}): {d['strat']['buy'][0]:,}원<br>2차: {d['strat']['buy'][1]:,}원<br>3차: {d['strat']['buy'][2]:,}원</div>
                    <div class="sell-box"><b>🔴 트리플 3분할 매도</b><br>1차(30%): {d['strat']['sell'][0]:,}원<br>2차(30%): {d['strat']['sell'][1]:,}원<br>3차(40%): {d['strat']['sell'][2]:,}원</div>
                </div>
