...
st.set_page_config(page_title="주식 비서 V64.9.2 Final", page_icon="🛡️", layout="wide")

# 프리미엄 UI 디자인 CSS (Full Spec 복구)
st.markdown("""
    <style>
...
def get_portfolio_gsheets():
    try:
        if not os.path.exists(".streamlit/secrets.toml") and not hasattr(st, "secrets"):
            st.sidebar.warning("⚠️ .streamlit/secrets.toml 설정 확인 필요")
...
# ==========================================
# 🖥️ 3. 메인 인터페이스 (사이드바 복구)
# ==========================================
with st.sidebar:
    st.title("🛡️ Hybrid V64.9.2")
    now_kst = get_now_kst()
    ...
    daily_report = st.checkbox("18시 마감 리포트 예약", value=True)
    st.markdown("---")
    ...
    if not shared_portfolio.empty:
        ...
        with st.spinner(f"{selected_name} 분석 중..."):
            ...
            if df_ai is not None:
                ...
                fig.add_trace(go.Scatter(x=d_tail.index, y=d_tail['BB_Upper'], line=dict(color='rgba(173,216,230,0.5)'), name="BB상단"))
                fig.add_trace(go.Scatter(x=d_tail.index, y=d_tail['BB_Lower'], line=dict(color='rgba(173,216,230,0.5)'), name="BB하단"))
        ...
        for idx, d in enumerate(sorted(found, key=lambda x: x['score'], reverse=True)[:10]):
            ...
            st.markdown(f"""<div class="scanner-card" style="border-left: 8px solid {acc_c};">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-top:10px;">
                    <div class="buy-box"><b>🔵 유기적 매수 (30:30:40)</b><br>1차({d['strat']['buy_names'][0]}): {d['strat']['buy'][0]:,}원<br>2차: {d['strat']['buy'][1]:,}원</div>
                    <div class="sell-box"><b>🔴 트리플 매도 (30:30:40)</b><br>1차: {d['strat']['sell'][0]:,}원<br>2차: {d['strat']['sell'][1]:,}원</div>
                </div>
            </div>""", unsafe_allow_html=True)
            if alert_scanner and idx < 3:
                has_scan = True; scan_msg += f"🔥 <b>{d['name']}</b>\n타점: {d['strat']['buy'][0]:,}원\n\n"
