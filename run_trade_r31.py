...
st.set_page_config(page_title="주식 비서 V62.2 KST-Hybrid", page_icon="⚡", layout="wide")

# 라이트 테마 및 사용자 커스텀 CSS 유지
st.markdown("""
    <style>
...
def get_hybrid_indicators(df):
    ...
    # OB(Order Block) 및 피보나치 등 분석 로직 유지
    ob_zones = []
            ...
            st.markdown(f"""
                <div class="scanner-card">
                    <div style="display:flex; justify-content:space-between;">
                        <h2>{icon} {d['name']}</h2>
                        <span style="color:#007bff; font-weight:bold; font-size:1.3em;">{d['score']:.1f}점</span>
                    </div>
                    <hr>
        ...
        if not match.empty:
            ...
            if df_bt is not None:
                ...
                if trades:
                    ...
                    st.plotly_chart(px.line(tdf['profit'].cumsum(), title="누적 수익률 추이", template="plotly_white"), use_container_width=True)
                else: st.warning("해당 조건으로 체결된 거래가 없습니다.")
